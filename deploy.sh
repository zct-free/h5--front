...
# 3. 检查是否存在已构建的h5-front目录
if [[ -d "h5-front" ]]; then
    log_info "发现已存在的构建目录 'h5-front'，将重新构建以确保使用最新代码"
else
    log_info "未发现构建目录 'h5-front'，将执行完整构建流程"
fi
...
# 6. 最终检查构建结果
if [[ ! -d "h5-front" ]]; then
    log_error "构建输出目录 'h5-front' 不存在"
    exit 1
fi

# 7. 创建压缩包
log_info "创建部署压缩包..."
tar -zcf "$ARCHIVE_NAME" h5-front/ || {
    log_error "创建压缩包失败"
    exit 1
}
