  ...
  methods: {
    ...
    async getIntegralDateList() {
      ...
      try {
        ...
        m = m < 10 ? `0${m}` : m;
        const params = {
          userId: this.$route.query.userId? this.$route.query.userId : window.localStorage.getItem("uuid"),
          startTime: `${y}-${m}-${d}`,
          endTime: `${this.today}`,
          pageNum: 1,
          pageSize: 30,
        };
        const { data: res } = await this.$axios.get(credits + "/days", {
          params,
        });
        const { code, msg, data } = res;
        if (code === 1000) {
          this.integralDateList = data.detailList;
          ...
          if (this.grade === "--") {
            ...
            this.grade = res.length ? res[0].booksNumber : this.grade;
          }
        } else {
          this.$toast.fail(msg);
        }
    ...
    async getCurrIntegralByDate() {
      try {
        ...
        const { data } = await this.$axios.get(credits + "/details", {
          params,
        });
    ...
    //   integralDateList currIntegralByDate
  },
};
</script>
