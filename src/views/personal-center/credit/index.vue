...
<template>
  <div class="personal-credit-container">
    <div class="header-container">
        ...
        <div @click="exchangeRecord" class="record">
          兑换记录
        </div>
    </div>
    <div class="content-container">
      <van-tabs v-model="active" @change="changeTab" title-active-color='#e22516'>
        <van-tab v-for="(item,index) in typeList" :key="index" :title="item.type_name" >
          <div class="content">
                <van-pull-refresh v-model="refreshing" @refresh="onRefresh">
                  ...
                  >
                    <div class="commodity-list">
                      <div class="commodity-item" @click="detail" v-for="(item, index) in detailList" :key="item.product_id">
                        <img :src="item.product_pic" alt="">
                        <p>{{ item.product_name }}</p>
                        <p>剩余数量：{{ item.store_count }}</p>
                        <div class="description">
                          <span class="number">{{ item.eb_count }}积分</span>
                          <van-button type="primary" size="mini" @click="exchange(index)">兑换</van-button>
                        </div>
                      </div>
                    </div>
                  </van-list>
...
const {SERVER_NAME: {credits}} = baseConfig
import {queryPrizesType, queryPrizesList, exchangePrizes} from '../../../../serve/api/integral'
export default {
    data:()=>({
        list:[],
        score:0,
    ...
    methods:{
      ...
      async getList(typeId){
        try {
          ...
          if( data.response.head.error_code === '00000'){
            ...
            const arr = data.response.body.proList.productVo.length ? data.response.body.proList.productVo : [data.response.body.proList.productVo]
      ...
      changeTab(val, title){
        ...
        let typeId = this.allTypes.find(item => item.type_name === title).type_code
        // console.log(this.allTypes);
